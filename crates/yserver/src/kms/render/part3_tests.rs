...
use crate::kms::{
    ...
    render::resources::{DrmCleanupRegistry, ResourceService, UseKind, tests::MockCleanupIo},
    vk::scanout::BoPhase,
};
...
fn c0_2ci_managed_scanout_flip_accepted_drm() {
    ...
    // by the pool BO's managed lease, not by this local read lease.
    ...
    let framebuffer_before_tick = device
        .get_crtc(output)
    ...
    // Wait only for the matching page-flip completion, with a hard deadline.
    // The production event hook drains the DRM fd and advances this exact BO
    // Pending -> OnScreen; no timing-only sleep or fixture bookkeeping is used.
    ...
    loop {
        ...
        // SAFETY: `poll_fd` is a valid stack value and `drm_fd` is owned by
        // the live fixture for the duration of this test.
        ...
        Backend::on_page_flip_ready(backend, &mut state, drm_fd);
    ...
    let current_framebuffer = device
        .get_crtc(output)
