...
## Update 2026-07-09 — the MoltenVK/1.2 gate + why rootless is the actual product

Three refinements from a later architecture pass.

**macOS Vulkan version — a non-issue: MoltenVK 1.3 supports Vulkan 1.3.** yserver requests `apiVersion 1.3` and enables two 1.3-core features — `dynamic_rendering` + `synchronization2` (`kms/vk/device.rs`). MoltenVK **1.3** (Khronos, 2024) advertises Vulkan 1.3 on recent macOS/Metal, and 1.3 conformance *mandates* both those features — so yserver's renderer runs on MoltenVK 1.3 **as-is**, no version gymnastics. (An earlier draft of this note assumed MoltenVK capped at 1.2 and concluded a KHR-extension downgrade was a macOS prerequisite — that was wrong; MoltenVK 1.3 obsoletes it.) The KHR-extension/uniform-dispatch downgrade is therefore **not** needed for the macOS port — it would only matter for older MoltenVK 1.2 on older macOS (niche). Confirm the actual MoltenVK build's advertised `apiVersion`/features on the target Mac when the port happens (same `vulkaninfo`-gate discipline as everywhere). Consequently **GH #87 ("support Vulkan < 1.3") loses its strongest justification** — macOS is covered by MoltenVK 1.3, and the Intel HD4000/Haswell it was filed for have no ANV Vulkan at all (→ Xorg or lavapipe). Windows native Vulkan is ≥1.3, unaffected.

**Rootless is the actual deliverable, because people run apps, not desktops.** X servers on Windows/macOS (VcXsrv, X410, XQuartz) exist to run *individual* apps — locally or, overwhelmingly, forwarded over `ssh -X`/`-Y` — never a full remote desktop. So rooted single-window (whole X screen in one host window) is the dev/test stepping-stone and first cut; the useful product is **rootless** (each top-level X window as a native host window) — the XWayland-rootless-class work flagged above as ~3× and "later". yserver's compositing architecture helps: every window already has GPU-backed storage, so rootless is "present each top-level's backing into its own host-window swapchain" — the images exist, unlike a non-compositing X server that would read pixels back. The hard part stays the X-window-semantics → host-window mapping (override-redirect popups, transients, stacking, input/focus to the host WM).

**The client/protocol/auth/SSH half is already done + proven on Linux.** `ssh -X -Y` into the Linux yserver works today (xauth cookie path). The port inherits all of it — dispatch, auth, xauth, remote clients — so the win/mac work is confined to: a host `Backend` impl, the WSI present layer, the rootless topology, and platform socket specifics (Windows historically has no unix-domain socket → TCP/named-pipe; macOS unix-socket like Linux). Nothing above the present layer changes. (`Backend: Send` was dropped in `b06383b6` / #86; it doesn't affect the `winit`-main-thread + spawned-`run_core` split — the backend stays on the run_core thread and `winit` only sends `HostInputEvent`s over the channel.)
