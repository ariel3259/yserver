...
`2026-09-20-phase-c0-stage-2c-iii-plan-ciii-identity.md`, which is implemented
and accepted **first**. They were never unflip work: they repair a one-device
assumption of stages 2c-i and 2c-ii, and keeping them here meant re-reviewing
the unflip every time the identity contract moved. This plan keeps the six
tasks that are the unflip and its route, renumbered 1-6; their content,
invariants, tests and mutation ids are unchanged, and the mutation numbering is
kept as it was so the two plans never collide. **This plan depends on plan
Ciii-identity being accepted**: every commit identity it records — the unflip
retirement's included — uses the `CommitKey`.
...
10. **Test names start with `c0_conv_ciii_`**, so one filter selects this plan.
    ...
    multi-device evidence of plan Ciii-identity is not built on it; `test_kms_device` plus
    ...
    Task 6 is named **outside** this prefix — `c0_hw_ciii_owner_route_on_card1_drm`,
...
- **This plan starts after plan Ciii-identity is accepted**
  (`2026-09-20-phase-c0-stage-2c-iii-plan-ciii-identity.md`): commit identity
  is a `CommitKey` there, and every identity this plan records uses it.
  Multi-device isolation is proven there too, so §6.2 is not this plan's.
...
None of these filters selects Task 6's hardware test: it is named outside the
...
**Portability (spec §8.4, round-1 M-3).** The **last task** (Task 6) and the coordinator
...
### Task 6: The hardware test of spec §6.4
...
4. the unflip back to composed, through Tasks 1-3.
...
After Task 6, with the user's go-ahead and the GPU free: the full hardware gate
