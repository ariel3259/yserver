...
- `c0_adm_conductor_drm_completion_batch_wakes_once` — a completion delivered through the DRM-event drain, not through `service_owner_completions` (use the drain path with a crafted record, or say why that is unreachable in a fixture), with another ready intent queued. The trace shows `Consumed → Enqueued → Decided → Dispatched`, with one wake after the drain's batch.
...
3. After Task 4: applies Q1–Q24 to your code; a survivor goes back as a finding.
