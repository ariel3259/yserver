...
**Revision 3 (2026-09-18)** — incorporates codex round 2 (`../findings/2026-09-18-stage-2c-ii-plan-b1-review-round2.md`: 2 blocking, 2 major, 1 minor, all verified and accepted; round 1 audited: four APPLIED, B-2 PARTIAL, closed by m-1 here).
- **B-1:** tier 3 is defined over the ready aged identities that would otherwise win, and a direct successor whose CRTCs carry stale or incompatible required maintenance is not admissible in any tier (C.0 §9.2.1: no plane-only successor).
- **B-2:** `confirm` consumes the symmetrically combined primary; `abort` keeps it.
- **M-1:** the oldest-compatible-primary test uses two legal shapes (composed and grouped direct) instead of two composed generations on one CRTC.
- **M-2:** tiers 2, 4 and 7 each get a `Waiting`-only negative scenario.
- **m-1:** `bound_violation`'s doc states the growing allowance.

Mutations P26–P30 were added.
...
- **Tier 3 is the retirement successor** ("the fairness-qualified version of submit-after-retirement", C.0 §9.2.1). It applies **only on a snapshot with `retirement_wake`**. It admits the queued direct successor if it is ready and eligible, the round-robin permits every CRTC it covers, and **every ready aged identity** — the ones that would otherwise win tier 4 — is compatible with it and lies on its CRTCs, so the successor absorbs them all. An aged identity that is `Waiting` would not win tier 4, so it does not hold tier 3 back (round-2 B-1). Otherwise tier 3 does not apply, and on ordinary wakes the direct successor competes in tier 6 as in A1.
- **No plane-only successor** (C.0 §9.2.1, round-2 B-1). If a maintenance identity on any of the successor's CRTCs has a pending generation the snapshot reports as not compatible with the successor (stale or incompatible), the successor is **not admissible in any tier**. It stays queued, and tier 4 services that maintenance when it is ready. Withdrawing or terminalizing the successor under its direct rules is the conductor's (B2) call. Readiness of the maintenance does not matter here: a stale required generation blocks the successor whether it is ready or not.
...
- **Symmetric absorption (tiers 4 and 7).** The winner combines the **oldest** ready primary on its CRTC that is compatible with it. It never combines a primary on a CRTC with a pending barrier, and never one the round-robin would refuse. The winner's own generation is carried. So is any other ready, compatible maintenance on that CRTC. *(Corrected during Task 2, 2026-09-18.)* In practice only **tier 4** can combine a primary. Tier 7 comes after tier 6. A ready primary on the winner's CRTC that the round-robin permits is taken by tier 6 first, and absorbs the maintenance itself (Task 3). One that the round-robin refuses cannot be combined either. So a tier-7 winner never finds a primary to combine: keep the rule, but expect it to combine nothing. **Tier 7 must never be evaluated before tier 6.** An earlier revision's test asked for a tier-7 combination, which is unreachable in the correct tier order.
...
- A2's conductor must keep compiling and passing, and must never dispatch a decision it cannot carry in full. **Before** it matches on `decision.admitted`, it aborts the token and returns `Unsupported(tier)`, with no dispatch and no state change, for any decision that has a non-empty `carried`, has a `combined_primary`, is one of the new `Admitted` variants, or has tier 4, 5 or 7. This includes a tier-6 composed or direct decision that carries maintenance (round-1 B-1): dispatching its primary alone would confirm the whole decision, spend the maintenance tickets and lose the payload, which has no home until B2. **A tier-3 decision that carries nothing is dispatched**: it is a plain `Admitted::Direct`, which is A2's own retirement promotion, and aborting it would switch that promotion off. *(Corrected during Task 3, 2026-09-18: revision 3 listed tier 3 among the aborted tiers, so Task 3's first implementation turned A2's retirement-promotion tests into "Unsupported" tests.)* Implementing the rest is B2's job.
...
| Tier 3 counts only ready aged identities; no plane-only successor when required maintenance is stale (round-2 B-1) | `c0_adm_maint_tier3_ignores_waiting_aged_identities`, `c0_adm_maint_stale_required_maintenance_blocks_the_successor_in_every_tier` | P26: `Waiting` aged identities block tier 3; P27: tier 6 admits a successor over stale required maintenance |
| `confirm` consumes the combined primary; `abort` keeps it (round-2 B-2) | `c0_adm_maint_confirm_consumes_the_combined_primary` | P28: `confirm` leaves the combined primary queued |
| No `Waiting` candidate wins tier 2, 4 or 7 (round-2 M-2) | `c0_adm_maint_waiting_candidates_never_win_tiers_2_4_7` | P29: drop the readiness guard of tier 4 or 7; P30: drop the readiness guard of the cursor recovery |
...
3. **Confirm** consumes the admitted intent (as today), **the `combined_primary` if there is one** (its slot is removed exactly as if it were admitted, and its CRTCs count as served for the round-robin — round-2 B-2), **and** every carried maintenance generation: its ticket is spent, its desired slot is emptied if the carried generation is still the desired one, and the identity is marked submitted with that generation. Confirm then ages every identity in `ages`. A barrier admission (topology, unflip, cursor recovery) ages every ready, unsent maintenance identity. Nothing resets a ticket.
3b. **Abort** leaves everything, the combined primary included, exactly as before `lock`.
...
**Named tests:** `c0_adm_maint_update_while_submitted_gets_a_new_ticket` (if not in Task 1), `c0_adm_maint_ages_after_losing_an_admission`, `c0_adm_maint_barrier_ages_overtaken_maintenance_without_resetting_tickets`, `c0_adm_maint_symmetric_absorption_combines_the_oldest_compatible_primary` (round-2 M-1: only one composed intent can exist per CRTC, so the two candidates are a composed intent on CRTC 1 and a grouped direct successor over {1, 2}, both ready and compatible with an **aged** cursor on CRTC 1 that wins tier 4. Run it twice, once with each shape older by `PrimaryOrdinal`: the older one is combined each time, and the carried set holds the cursor), `c0_adm_maint_symmetric_absorption_respects_barriers_and_round_robin` (a compatible primary on a CRTC with a pending unflip is not combined; nor is one whose CRTC was just served while another is owed), `c0_adm_maint_cursor_recovery_is_a_tier2_barrier` (a ready cursor recovery wins over an older ready composed on another CRTC; a composed on its own CRTC does not compete), and a tier-4-over-tier-7 test of your naming. Also: `c0_adm_maint_symmetric_absorption_skips_an_incompatible_primary` — the older composed on the winner's CRTC is incompatible and a younger one is compatible, so the younger is combined; with none compatible, none is. The combines test gets a second case, where another ready, compatible maintenance identity on that CRTC rides along in `carried`. And `c0_adm_conductor_maintenance_carrying_tier6_is_unsupported` — in A2, a tier-6 composed decision that carries a gamma: the conductor returns `Unsupported`, the source's `describe` and `composed_resources` were never called, the owner's slot is free, and the decider is unlocked with the gamma still queued. Also: `c0_adm_maint_confirm_consumes_the_combined_primary` — a tier-7 cursor with a combined composed primary, through real `lock` / `confirm`: the composed slot is empty and CRTC 1 counts as served; the same fixture through `lock` / `abort` leaves the composed intent queued. And `c0_adm_maint_waiting_candidates_never_win_tiers_2_4_7` — three fixtures: a cursor recovery reported `Waiting`, an aged identity reported `Waiting`, and a non-aged identity reported `Waiting`; in each, that candidate is not chosen, and a lower tier or nothing is.
...
impl Admission {
    ...
    /// admissions than `2 ×` the distinct identities with an older ticket that
    /// have been aged at any moment since it aged (the allowance only grows).
...
3. After Task 4: applies P1–P30 to your code and records which tests fail; a survivor goes back as a finding.
