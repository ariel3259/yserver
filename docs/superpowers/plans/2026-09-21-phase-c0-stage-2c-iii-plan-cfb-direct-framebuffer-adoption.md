...
**Revision 5 (2026-09-21, coordinator)** — Task 2's F8: `c0_conv_cfb_late_cleanup_failure_keeps_the_final_role` needs an allocation that completed its roles and dropped its last lease, which exists only once Task 4's handoff does (`commit.rs` still finishes the role directly). The test and its mutations F20b/F20c **move to Task 4**; Task 2 delivers the owner, the `FinalRole` charge variant and the two adoption-time tests.

**Revision 4 (2026-09-21)** — incorporates codex round 3 (`../findings/2026-09-21-stage-2c-iii-plan-cfb-review-round3.md`: 2 blocking, 1 minor, coverage complete; round 2: M-1, M-2, M-3 APPLIED, B-1 PARTIAL, closed here).
- **B-1:** two failure paths, two owners: a failed cleanup **after adoption** keeps the `Preparing` charge; a failed cleanup **after normal role completion** keeps the allocation's **final role** (`OrdinaryRetirement` or `ExitRetirement`) occupied until cleanup succeeds or the teardown barrier takes it — no new reservation. Task 2 tests both, the second with `Preparing` occupied.
- **B-2:** Phase A results are **staged in the service**, never returned to a caller that could drop them; the backend alone takes and acknowledges them; before any payload is transferred for cleanup the service **revalidates** that it has no live lease and no obligation, atomically. F31 covers the scene sites; F33 covers remint-before-authorization.
- **m-1:** the plan cites spec revision 5.
...
6. **The adoption transaction** (2.6): adoptability check → `into_managed` (payload is sole owner) → service adopt; a service failure goes to registry cleanup and closes admission; a cleanup failure goes to a keyless, alias-counted pending-cleanup entry with retry / freeze-handoff / family-closed dispositions. **Two charges, by origin** (round-3 B-1): a cleanup that fails right after adoption keeps the `Preparing` charge; a cleanup that fails after the allocation completed its roles keeps its **final role** (`OrdinaryRetirement` or `ExitRetirement`) occupied — `finish_role` is deferred until cleanup succeeds or the teardown barrier takes the entry — so bounded accounting holds without reserving anything new, and admission stays gated by that occupied role as 2c-i §6 already specifies for uncertain cleanup.
...
11. **One authoritative service step, two phases, durable handoff** (round-1 B-2, round-2 B-1, round-3 B-2). *Phase A, in the service:* when a lease drop takes a key's live-use count to zero, the service records a typed **zero edge** for that key, **staged inside the service** (not returned to the caller): `service_completions` and readiness stage edges and never destroy a payload; a caller that ignores their `Result` — the scene at `scene.rs:3915`/`:3965` — drops nothing, because there is nothing to drop. A staged edge survives a lease minted after the drop. *Phase B, in the backend:* exactly one function **takes** the staged edges (the only consumer; taking acknowledges them) — called **after** `route_owner_event_batch` at the two backend sites (`backend.rs:21704`, `:21715`, which today run before it) and right after the scene's drain returns — which: (a) removes every M1 index token for the taken edges; (b) then asks the service to transfer for cleanup only those keys that it **revalidates at that moment** as having no live lease and no pending obligation (atomically with the transfer; an earlier "releasable" is never trusted); (c) authorizes the registry's cleanup of exactly those. It runs before any managed lookup in the same tick and before an incarnation's teardown handoff. F31 is applied at each of the four sites independently; F33 covers a lease minted between the edge and the authorization.
...
| The service step takes the staged zero edge, removes the index, revalidates, then authorizes cleanup, at every site (decision 11) | `c0_conv_cfb_service_step_removes_the_index_before_cleanup_vulkan` (four cases, one per site, including a lease dropped by an owner event routed at that site and one dropped inside the scene's own `service_completions` call) | F31: at one site, run the step before `route_owner_event_batch` again / return edges to the caller instead of staging them (applied per site, the scene sites included); F32: destroy in readiness before the index is removed; F33: trust an earlier "releasable" — transfer for cleanup without revalidating after a lease was minted between the edge and the authorization |
...
| A failed cleanup (forced `RMFB`, forced `GEM_CLOSE`) retains the payload with its charge by origin; a later retry releases once (2.6, decision 6) | `c0_conv_cfb_failed_cleanup_is_retained_and_retried` (two cases), `c0_conv_cfb_late_cleanup_failure_keeps_the_final_role` | F19: drop the payload on cleanup failure; F20: release the `Preparing` charge before cleanup succeeds; F20b: `finish_role` before a late cleanup succeeds; F20c: reserve a new role for a late-failure entry |
...
- A payload whose cleanup failed is retained as a **pending-cleanup entry**: keyless, holding the payload with its right and device alias, counted in the incarnation's alias total exactly like an adopted payload alias, and holding **its charge by origin**: the `Preparing` charge after Task 1's step-3 failure; the allocation's final role (`OrdinaryRetirement`/`ExitRetirement`, `finish_role` deferred) after a normal-completion cleanup failure. No new reservation is ever made for an entry.
...
- **The two-phase service step** (decision 11): the service stages zero edges at lease drop and never destroys; the backend step is the only taker, runs after `route_owner_event_batch` at both backend sites and after the scene's drain, removes the index for each taken edge (and on incarnation loss and on the source drawable's drop, the existing `scanout_m1.remove` site), then transfers for cleanup only what the service revalidates as lease-free and obligation-free at that moment. A staged edge survives a lease minted after the drop; such a key is not cleaned.
...
After each task: read the diff, grep for `owner_route`-style branches inside Legacy bodies, re-run the checks outside codex, apply this plan's mutations F1–F35 (with F9a–c, F12a–b, F20b–c) **by line** against the recorded mutation text, confirm each is caught by its named test (or record it equivalent with the reason), then commit with `Implemented-By: codex (model gpt-5.6-luna, reasoning effort xhigh)`. `_vulkan` tests and their mutations run on the GPU with the user's go-ahead, from tty when the user asks for it.
