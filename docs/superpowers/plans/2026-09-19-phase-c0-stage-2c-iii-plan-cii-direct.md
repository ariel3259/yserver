...
6. **Two different samples, told apart (round-1 B-1, round-2 B-1).** A direct frame carries `completion_clock: Option<PresentClockSample>`, the sample of its reference CRTC (`backend.rs:588`-`603`). Two rules, not one:
   ...
   - **Absence has a runtime rule, not an F8.** If the reference CRTC has no validated sample, the owner route treats it as an invariant failure: close the device's transport through the existing path, and let the drain terminalize what is in flight. The implementer also measures whether an accepted direct commit can exist on a CRTC that never recorded a validated sample, and reports the answer with the code that supports it.
...
| The `Flip` publication uses only this commit's `Presented` for this frame's reference CRTC (decision 6) | `c0_conv_cii_direct_present_completes_once_vulkan` | S25: accept another commit's sample as this frame's `Flip` sample |
| A CRTC with no validated sample never publishes a fabricated `(0, 0)` (decision 6) | `c0_conv_cii_direct_missing_presented_without_history_vulkan` | S26: publish `present_get_completion_clock`'s default |
...
**Named tests:** `c0_conv_cii_direct_description_carries_present_vulkan`, `c0_conv_cii_direct_present_completes_once_vulkan` (both event orders), `c0_conv_cii_direct_missing_presented_skips_vulkan` (distinct samples per CRTC, so a `Skip` stamped from **another CRTC** fails it — while a sample from an **earlier commit on the same CRTC** is the correct fallback and must pass), `c0_conv_cii_direct_missing_presented_without_history_vulkan` (a reference CRTC with no validated sample: the transport closes and nothing is published with `(0, 0)`; the report says whether that state is reachable at all), and `c0_conv_cii_stale_presented_does_not_move_the_clocks_vulkan` (a `Presented` for an unknown or stale `CommitId` leaves the platform's clocks where they were, so a later `Skip` is stamped from the sample a real commit recorded).
