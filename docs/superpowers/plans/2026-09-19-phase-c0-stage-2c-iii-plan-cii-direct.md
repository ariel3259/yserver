...
**Revision 6 (2026-09-20)** — Task 7's composed-invalidation invariant corrected against a third F8 stop, verified by the coordinator.
The plan (following spec §5.5 and DMG-5, "as the merged base already does") said direct **entry** invalidates the affected outputs' composed buffers. It does not: in this checkout the only device-wide invalidation on the direct path is on the **return**, at composed-unflip retirement (`backend.rs:3374`, `scene.rs:1500`'s doc comment names exactly that caller and `set_logical_screen_size`). Entry needs none, because while an output scans out directly its composed buffers are not painted and are not read until the return, which invalidates them. **Spec correction, recorded here:** DMG-5's entry half does not describe the merged base. What Cii owes is therefore only that **no direct milestone applies composed damage**; the return-path invalidation belongs to Ciii, which converts the unflip, and Ciii's plan must carry it.

**Revision 5 (2026-09-20)** — decision 6 corrected against two F8 stops the implementer raised in Task 6, both verified by the coordinator in the code.
- **The clock probe cannot supply a stamp.** `ProbeOutcome::Ready { reference }` carries a **sequence reference**, not a sample; `Clock::install_reference` sets that reference and `page_sample(raw, sec, usec)` still needs a real page event to produce `(msc, ust)`. Revision 4's readiness precondition — "ask for a clock probe and admit when it resolves" — is therefore impossible as written, and it is **removed**.
- **The missing-`Presented` case cannot reach retirement.** `CommitRecord::into_completed` asserts "cannot complete without Presented for Present CRTCs" (`owner/record.rs:400`), so a Present-carrying commit never reaches `CompletionRetired` without its `Presented`; it resolves through the completion deadline into `CompletionUnknown` (C.0 §10). The `Skip` therefore belongs to that **terminal path**, not to retirement.
- **What a `Skip` is stamped with.** The last validated sample of the frame's reference CRTC **if one is known** (the fallible accessor, not `present_get_completion_clock`'s fabricated `(0, 0)`), and otherwise **unstamped**, exactly as today's legacy `defer_direct_successor_skip` publishes a never-submitted successor's `Skip` with no sample at all (`backend.rs:2761`). Nothing is fabricated, and no new failure mode is invented.

**Revision 4 (2026-09-19)** — incorporates codex round 3 (`../findings/2026-09-19-stage-2c-iii-plan-cii-review-round3.md`: 1 blocking, 2 major, all verified against the tree and accepted; round 2: M-1 PARTIAL and B-1 TRADED, both closed here).
- **B-1:** revision 3's "close the transport and publish nothing" was not a terminalization: `force_close` only marks the gate (`resources/transport.rs:373`), while C.0 §10.4 requires every Present to reach a protocol terminal state and unpark the FIFO. The author also measured that the case is genuinely reachable — under `Owner` a composed commit carries no Present, so it records no completion clock, and a device's first direct Present can find an empty history. Decision 6 now makes a **validated epoch-local sample a readiness precondition**, obtained through the owner's existing clock probe (`begin_clock_probe`, `device.rs:1971`; `OwnerEvent::ClockProbeResolved`) when the reference CRTC has none. Nothing is admitted without one, so the no-sample `Skip` cannot arise and no failure mode is invented.
- **M-1:** S30 stays unkillable unless the named test actually calls the old entry; Task 1 gains that third case.
- **M-2:** the `Flip` sample must be proven on both correlation dimensions; Task 6's test injects distinct samples for the reference and non-reference CRTCs and a foreign commit, and S31 selects the non-reference one.

**Revision 3 (2026-09-19)** — incorporates codex round 2 (`../findings/2026-09-19-stage-2c-iii-plan-cii-review-round2.md`: 1 blocking, 1 major, both verified against the tree and accepted; round 1: four APPLIED, B-1 TRADED and closed here).
- **B-1:** decision 6 conflated two different samples and leaned on an API that cannot express absence. It now separates the **commit-bound** sample (this commit's `Presented`, for the `Flip` publication) from the **per-CRTC historical** sample (which the spec's `Skip` fallback explicitly allows to come from an earlier commit), requires a fallible lookup instead of `present_get_completion_clock`'s fabricated `(0, 0)` (`platform.rs:5241`-`5250`), gives absence a runtime rule rather than an F8, and adds the correlation-before-clocks invariant: today a `Presented` updates the platform's clocks before the commit is correlated (`backend.rs:19898`-`19914`), which §3.4 forbids.
- **M-1:** Task 1's new contracts had no rows in the exit table. S25-S27 cover confirming instead of aborting, registering under a different `CommitId`, and letting `begin_with_ledger` accept a Present-carrying description.
...
- **B-1:** a missing `Presented` had no defined carrier for the clock sample its `Skip` must use. Decision 6 names the source, the binding and the F8 when no validated sample exists; Task 6 tests it with distinct samples per CRTC.
...
6. **Two different samples, told apart (round-1 B-1, round-2 B-1).** A direct frame carries `completion_clock: Option<PresentClockSample>`, the sample of its reference CRTC (`backend.rs:588`-`603`). Two rules, not one:
   - **The commit-bound sample.** The `Flip` publication uses only the sample that **this commit's** `Presented` delivered for **this frame's reference CRTC**, bound by `(CommitId, reference CRTC)`. Another CRTC's sample, or another commit's, may never produce a `Flip`.
   - **The historical fallback.** An accepted Present that reaches retirement without one terminalizes as `Skip` with the **last validated sample of that CRTC**, which the spec deliberately allows to be older and to come from an earlier commit (stage 2c §3: "the last validated clock sample"). The lookup must be **fallible**: `present_get_completion_clock` (`platform.rs:5241`) returns a fabricated `(msc: 0, ust: 0)` when the CRTC has none, and that value may never be published. Add or use an accessor that reports absence.
   - **Where the `Skip` happens, and what stamps it (revision 5).** A Present-carrying commit cannot retire without its `Presented` (`owner/record.rs:400`), so the "accepted Present lacking validated presentation" of stage 2c §3 resolves through the completion deadline into `CompletionUnknown` (C.0 §10). The frame's `Skip` is published from that terminal path, stamped with the last validated sample of its reference CRTC **when one is known** — through an accessor that reports absence, never `present_get_completion_clock`'s fabricated `(0, 0)` — and **unstamped when none is**, which is exactly what the legacy path already does for a never-submitted successor's `Skip` (`backend.rs:2761`). There is no clock-probe precondition: the probe yields a sequence reference, not a sample.
   - **Correlation before clocks.** Today `route_owner_event`'s `Presented` arm writes `record_vblank_clock` and `record_completion_clock` for every sample **before** the commit is correlated (`backend.rs:19898`-`19914`); §3.4 says a consumer that sees an unknown `CommitId` ignores it. On the owner route the samples of an unknown or stale commit must not reach the platform's clocks, or a stray event silently rewrites the fallback that a live frame will be stamped with.
...
| A direct registration failure aborts the token and consumes no admission state (§4.0, round-2 M-1) | `c0_conv_cii_direct_registration_failure_aborts_the_token` | S28: confirm instead of abort on that path |
| Obligations are registered under the record's own `CommitId` (§3.2, round-2 M-1) | `c0_conv_cii_direct_registration_failure_aborts_the_token` | S29: register under a different `CommitId` |
| `begin_with_ledger` still refuses Present-carrying descriptions (§5.0, round-2 M-1) | `c0_conv_cii_present_entry_registers_and_refuses` | S30: let it accept them |
...
| A Present whose commit terminalizes as `CompletionUnknown` publishes `Skip` with the last validated sample of its reference CRTC, which may be older and from an earlier commit (§3.3, decision 6, revision 5) | `c0_conv_cii_direct_unknown_terminal_skips_vulkan` | S16: publish `Flip` with a synthesized timestamp; S17: stamp the `Skip` from another CRTC's sample |
| The `Flip` publication uses only this commit's `Presented` for this frame's **reference** CRTC (decision 6; round-3 M-2) | `c0_conv_cii_direct_present_completes_once_vulkan`, driven with distinct samples for the reference and non-reference CRTCs and a foreign commit's `Presented` | S25: accept another commit's sample; S31: stamp from the non-reference CRTC's sample |
| A reference CRTC without a validated sample publishes its `Skip` unstamped, never a fabricated `(0, 0)` (decision 6, revision 5) | `c0_conv_cii_direct_skip_without_history_is_unstamped_vulkan` | S26: stamp it from `present_get_completion_clock`'s default |
| An unknown or stale commit's `Presented` does not touch the platform's clocks (decision 6, §3.4) | `c0_conv_cii_stale_presented_does_not_move_the_clocks_vulkan` | S27: write the clocks before correlating the commit, as today |
...
| Direct entry invalidates nothing, matching Legacy; the return path is Ciii's (§5.5 as corrected by revision 6) | `c0_conv_cii_direct_entry_matches_legacy_invalidation_vulkan` | S21: invalidate the composed buffers on direct entry in the owner route only |
...
**Named tests:**
- `c0_conv_cii_present_entry_registers_and_refuses` — three cases: a Present-carrying description accepted through the new entry, whose record carries the expected completion CRTCs; a **structurally valid** description with an independently invalid `CompletionContext`, refused **before the ledger closure runs** (round-1 M-1: a consumer outside the kernel event set is refused by closure construction, `closure.rs:215`, so it cannot witness S1); and the **old** `begin_with_ledger` entry, called with an otherwise valid Present-carrying description, still refusing it without running the closure or touching the owner (round-3 M-1, mutation S30).
...
**Named tests:** `c0_conv_cii_direct_description_carries_present_vulkan`, `c0_conv_cii_direct_present_completes_once_vulkan` (both event orders, and — round-3 M-2 — distinct samples for the reference and non-reference CRTCs of a grouped commit plus a foreign commit's `Presented`, so only this commit's reference sample can produce the `Flip`), `c0_conv_cii_direct_missing_presented_skips_vulkan` (distinct samples per CRTC, so a `Skip` stamped from **another CRTC** fails it — while a sample from an **earlier commit on the same CRTC** is the correct fallback and must pass), `c0_conv_cii_direct_unknown_terminal_skips_vulkan` (the commit reaches `CompletionUnknown` through the completion deadline and its `Skip` is stamped from the reference CRTC's last validated sample) and `c0_conv_cii_direct_skip_without_history_is_unstamped_vulkan` (the same path with no validated sample at all: the `Skip` is published unstamped, as the legacy never-submitted path does, and nothing carries `(0, 0)`), and `c0_conv_cii_stale_presented_does_not_move_the_clocks_vulkan` (a `Presented` for an unknown or stale `CommitId` leaves the platform's clocks where they were, so a later `Skip` is stamped from the sample a real commit recorded).
...
**Invariants (spec §5.5, §5.6, as corrected by revision 6):**
- **No milestone of a direct transaction applies composed damage** — not `Accepted`, not `HardwareComplete`, not the terminal path. A direct commit's outcome never stages, applies or acks a composed output's damage.
- **Direct entry itself invalidates nothing**, which is what the legacy route does: the invalidation happens on the return, at composed-unflip retirement, and that path is Ciii's. Prove the owner route matches legacy here rather than adding an invalidation legacy does not perform.
- A direct commit never carries an unchanged cursor generation, and a primary flip event does not retire a cursor generation newer than the one its commit carried; that one is retired only by its own commit.

**Named tests:** `c0_conv_cii_direct_entry_matches_legacy_invalidation_vulkan` (the composed damage state after an owner-route direct entry equals the state after the same entry on `Legacy`),`c0_conv_cii_direct_milestones_leave_composed_damage_vulkan`, `c0_conv_cii_direct_never_carries_an_unchanged_cursor`, and `c0_conv_cii_primary_flip_does_not_retire_a_newer_cursor_vulkan` (round-1 M-4, mutation S24) — a cursor generation newer than the one the completing primary commit carried is still live after that commit's milestones, and only its own commit retires it.
