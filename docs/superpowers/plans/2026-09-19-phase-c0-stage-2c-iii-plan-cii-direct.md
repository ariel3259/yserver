...
6. **Two different samples, told apart (round-1 B-1, round-2 B-1).** A direct frame carries `completion_clock: Option<PresentClockSample>`, the sample of its reference CRTC (`backend.rs:588`-`603`). Two rules, not one:
   ...
   - **A validated sample is a readiness precondition, so absence cannot arise (round-3 B-1).** The case is reachable — under `Owner` a composed commit carries no Present and records no completion clock, so a device's first direct Present can meet an empty history — and "close the transport and publish nothing" is not a terminalization: `force_close` only marks the gate (`resources/transport.rs:373`), while C.0 §10.4 requires every Present to reach a terminal state and unpark the FIFO. So a direct intent whose reference CRTC has no validated epoch-local sample is **`Waiting`, not `Ready`**: the conductor asks the owner for a clock probe (`begin_clock_probe`, `device.rs:1971`) and the intent becomes ready when `OwnerEvent::ClockProbeResolved` supplies the sample. Nothing is admitted without one, no `(0, 0)` is ever published, and a probe that resolves unavailable leaves the intent `Waiting` rather than yielding a fabricated stamp.
...
| A missing `Presented` terminalizes as `Skip` with the last validated sample of its reference CRTC, which may be older and from an earlier commit (§3.3, decision 6) | `c0_conv_cii_direct_missing_presented_skips_vulkan` | S16: publish `Flip` with a synthesized timestamp; S17: stamp the `Skip` from another CRTC's sample |
...
| A reference CRTC without a validated sample keeps the intent `Waiting` until the clock probe resolves; nothing is admitted and no `(0, 0)` is published (decision 6; round-3 B-1) | `c0_conv_cii_direct_waits_for_a_validated_clock_vulkan` | S26: report the intent `Ready` without a validated sample; S32: publish `present_get_completion_clock`'s default at retirement |
